...
<?php
$display = false;
$viewed = "<a class='text-white' href='?most-viewed&range'>Most Viewed</a>";
$audio = "<a class='text-white' href='?audio&range'>Audio</a>";
$most = "<a class='text-white' href='?most-rated&range'>Most Rated</a>";
$top = "<a class='text-white' href='?top-download&range'>Top Download</a>";
$home = "<a class='text-white' href='?index.php'>Home</a>";
if(isset($_GET['audio'])){
$audio = "Audio";	
$recent = $audio;
$selected = "?audio";
}elseif(isset($_GET['most-rated'])){
$most = "Most Rated";
$recent = $most;	
$selected = "?most-rated";
}elseif(isset($_GET['top-download'])){
$top = "Top Download";
$recent = $top;	
$selected = "?top-download";
}elseif(isset($_GET['most-viewed'])){
$viewed = "Most Viewed";
$recent = $viewed;	
$selected = "?most-viewed";
}elseif(isset($_SERVER['PHP_SELF'])){
$home = "Home <span class='badge btn btn-dark btn-sm disabled' aria-disabled='true'></span>";	
$recent = $home;
$display = "none";
}
...
?>
<nav>
<div class="dropdown p-1 btn-group">
  <a class="btn-sm btn btn-secondary dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    <?php echo $recent;?>
  </a>
  <div class="dropdown-menu bg-secondary" aria-labelledby="dropdownMenuLink">
  <ul class="navbar-nav mr-auto p-1" id="navbarSupportedContent">
	<li  aria-current="page"><?php echo $home?></li>
    <li  aria-current="page"><?php echo $audio?></li>
	<li  aria-current="page"><?php echo $top?></li>
	<li  aria-current="page"><?php echo $most?></li>
	<li  aria-current="page"><?php echo $viewed?></li>
  </ul>
  </div>
  <div class="dropdown ml-1" style='display:<?php echo $display?>;'>
  <div class="dropdown-menu bg-secondary" aria-labelledby="dropdownMenuLink">
  <ul class="navbar-nav mr-auto p-1" id="navbarSupportedContent">
	<?php 
		$all = "<a class='text-white' href='$selected&range'>All</a>";
		$day7 = "<a class='text-white' href='$selected&range=7'>Last 7 Days</a>";
		$day14 = "<a class='text-white' href='$selected&range=14'>Last 14 Days</a>";
		$day28 = "<a class='text-white' href='$selected&range=28'>Last 28 Days</a>";	
		if($_GET['range'] == false){
		$all = "All";
		$recent = "All";		
		}elseif($_GET['range'] == 7){
		...
		$day14 = "Last 14 Days";
		$recent = "Last 14 Days";		
		}elseif($_GET['range'] == 28){
		$day28 = "Last 28 Days";
		$recent = "Last 28 Days";		
		}
	?>
	<li  aria-current='page'><?php echo $all?></li>
	<li  aria-current='page'><?php echo $day7?></li>
	<li  aria-current='page'><?php echo $day14?></li>
	<li  aria-current='page'><?php echo $day28?></li>
  </ul>
  </div>
  ...
  <div class="p-1">
<b class=""><a class="text-danger" href="?schedule">* Schedule</a></b>
</div>  
</div>
<div class="collapse" id="collapseExample">
    <form action="search.php" method="get" class="form-inline my-2 my-lg-0 ml-1">
      <input style="height:30px;width:591px;" name="search" class="form-control" type="search" placeholder="Search" aria-label="Search">
    </form>
</div>
...
$ds = "block";	
?>	
<table class="table table-bordered table-hover table-responsive" style="display:<?php echo $ds ?>;text-align:center">
<caption>Schedule List</caption>
  <thead>
    <tr>
...
$timesat = $row['timesat'];
$user = $row['user'];
$sqls = "SELECT * FROM users WHERE token LIKE '%$user%'";
$results = $connsource->query($sqls);
while($rows = $results->fetch_assoc()){
?>
  <tbody>
    <tr>
	  <th scope="row"><i><?php echo $name = $rows['name'];?></i></th>
      <td class="text-success"><?php if($timesun != ""){ echo $timesun;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>
      <td class="text-success"><?php if($timemon != ""){ echo $timemon;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>
      <td class="text-success"><?php if($timetue != ""){ echo $timetue;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>
	  <td class="text-success"><?php if($timefou != ""){ echo $timefou;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>
	  <td class="text-success"><?php if($timefif != ""){ echo $timefif;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>
	  <td class="text-success"><?php if($timefri != ""){ echo $timefri;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>
	  <td class="text-success"><?php if($timesat != ""){ echo $timesat;}else{echo "<i class='fa fa-close text-danger'></i>";}?></td>	  
    </tr>
  </tbody>
<?php
}
...
$date = new DateTime();
$date->modify('-7 day');
$day7 = $date->format('m.d.y');
$date = new DateTime();
$date->modify('-14 day');
$day14 = $date->format('m.d.y');
$date = new DateTime();
$date->modify('-28 day');
$day28 = $date->format('m.d.y');
if(isset($_GET['most-viewed']) && isset($_GET['range'])){
if($_GET['range'] <= 28){	
$sql = "SELECT * FROM videos WHERE views_counter AND date<='$day28' ORDER BY views_counter DESC";	
}
if($_GET['range'] <= 14){	
$sql = "SELECT * FROM videos WHERE views_counter AND date<='$day14' ORDER BY views_counter DESC";	
}
if($_GET['range'] <= 7){	
$sql = "SELECT * FROM videos WHERE views_counter AND date<='$day7' ORDER BY views_counter DESC";	
}
if($_GET['range'] == false){	
$sql = "SELECT * FROM videos WHERE views_counter ORDER BY views_counter DESC";	
}
}elseif(isset($_GET['top-download']) && isset($_GET['range'])){
if($_GET['range'] <= 28){	
$sql = "SELECT * FROM videos WHERE downloads AND date<='$day28' ORDER BY downloads DESC";	
}
if($_GET['range'] <= 14){	
$sql = "SELECT * FROM videos WHERE downloads AND date<='$day14' ORDER BY downloads DESC";	
}
if($_GET['range'] <= 7){	
$sql = "SELECT * FROM videos WHERE downloads AND date<='$day7' ORDER BY downloads DESC";	
}
if($_GET['range'] == false){	
$sql = "SELECT * FROM videos WHERE downloads ORDER BY downloads DESC";	
}	
}elseif(isset($_GET['most-rated']) && isset($_GET['range'])){	
if($_GET['range'] <= 28){	
$sql = "SELECT * FROM videos WHERE likes AND date<='$day28' OR deslikes AND date<='$day28' ORDER BY likes OR deslikes DESC";	
}
if($_GET['range'] <= 14){	
$sql = "SELECT * FROM videos WHERE likes AND date<='$day14' OR deslikes AND date<='$day14' ORDER BY likes OR deslikes DESC";	
}
if($_GET['range'] <= 7){	
$sql = "SELECT * FROM videos WHERE likes AND date<='$day7' OR deslikes AND date<='$day7' ORDER BY likes OR deslikes DESC";	
}
if($_GET['range'] == false){
$sql = "SELECT * FROM videos WHERE likes OR deslikes  ORDER BY deslikes OR likes DESC";		
}
}elseif(isset($_GET['audio']) && isset($_GET['range'])){
if($_GET['range'] <= 28){	
$sql = "SELECT * FROM videos WHERE audio AND date<='$day28' ORDER BY audio DESC";	
}
if($_GET['range'] <= 14){	
$sql = "SELECT * FROM videos WHERE audio AND date<='$day14' ORDER BY audio DESC";	
}
if($_GET['range'] <= 7){	
$sql = "SELECT * FROM videos WHERE audio AND date<='$day7' ORDER BY audio DESC";	
}
if($_GET['range'] == false){	
$sql = "SELECT * FROM videos WHERE audio ORDER BY audio DESC";	
}
}
$result = $connsource->query($sql);
if(isset($result->num_rows)) {
    ...
    while($row = $result->fetch_assoc()) {
        $id = $row['id'];
			...
			$contentspace = str_replace(' ', '', $content);
			$out = strlen($content) > 50 ? substr($content,0,100)."<a style='color:#fe9090;' href='display.php?v=$out_title'>...</a>" : $content;
			...
			$extension = pathinfo($row['audio'],PATHINFO_EXTENSION);
			$extensionimage = pathinfo($row['image'],PATHINFO_EXTENSION);
			$image = $row['image'];
			...
			$total = $likes + $deslikes;
			$percent = round(@($likes / $total) * 100);
			$bar = "<div class='progress bg-danger' style='height:2px'>
...
$today = date("m.d.y");
if ($date == $today){
	$new = "<div class='badge text-white' style='background-color:#91e842;box-shadow: 1px 1px black'>New</div>". " ";
...
<li class="media">
<!--THUMBNAIL-->
<a class=""  href="display.php?v=<?php echo $out_title?>">
<div>
<img class="p-1" width='100' height='80' src='<?php if ($extensionimage != "jpg" && $extensionimage != "png" && $extensionimage != "gif"){echo "pod.jpg";}else{echo $image;}?>'><i style="position:absolute;left:51px;margin-top:21px;font-size:2.0vw;color: rgba(0, 0, 0, .5);" class="fas fa-play"></i></img>	
</div>
<div class="p-1">
</a>
<!--THUMBNAIL-->
...
<h5 class="text-secondary" style="font-size:15px;"><?php if($extension == "mp3"){echo "<i style='color:#00B7EF;' class='fas fa-microphone'></i>";}else{echo "<i style='color:#FF8902;' class='fa fa-coffee'></i>";}?> <a class="text-secondary" href="display.php?v=<?php echo $out_title?>"><?php echo $title ?></a> <span class='text-dark'><?php echo $comments?> Comments</span><br>
<div style="min-height:36px;min-width:490px"><?php echo $out?><br></div>
<i class="text-dark fa fa-calendar"></i> <?php echo $date ?> 
<i class='text-dark fas fa-user'></i> <a class="text-secondary" href="staff.php?uploader=<?php echo $out_uploader?>"><?php echo $uploader ?></a> 
<i class='text-dark fas fa-tag'></i> <a class="text-secondary" href="search.php?search=<?php echo $pieces[0]?>"><?php echo $pieces[0]?></a>
<i class='text-dark fa fa-eye'></i> <?php echo $views_counter?> 
<i class='text-dark fas fa-download'></i> <?php echo $downloads?>
<?php echo $bar;?>
</h5>
</li>
</div>
...
echo "<div class='btload d-flex justify-content-center mb-1'>
<a href='#' class='btn btn-outline-secondary btn-sm ml-1 mr-1 mt-1 btn-block ' id='loadMore'>Load More</a>
</div>";
}
}
if($result->num_rows == 0){
echo "<h1 class='col' style='color:pink;text-align:center'><i class='far fa-angry'></i> Apparently there aren't any results yet, comeback later.</h1>";
}

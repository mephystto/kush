...
$sql10 = "SELECT id, title, content, date, tag, video_name, views_counter, uploader FROM videos WHERE video_name LIKE '%$searchq%'";
$res = mysqli_query($db, $sql10) or die(msqli_error());
if($_GET['v'] == false || $res->num_rows == 0){
...
if(isset($_POST['like']) || isset($_POST['deslike'])){
	...
	header( "refresh:0.1;url=?v=$name$page" );
...
<body style="background:url('wallpaper.jpg');background-repeat: no-repeat;background-size: cover;background-attachment: fixed; ">
<div class="progress bg-danger" id="progress" style="position:fixed;top:0px;"></div>
<style>
.main{
	margin: auto;
  width: 600px;
...
if (mysqli_num_rows($res) > 0) {
    ...
    while($row = $res->fetch_assoc()) {
			...
			$likes = $row['likes'];
			$deslikes = $row['deslikes'];
			$audio = $row['audio'];
			//convert to link or image
			$url = '@(http(s)?)?(://)?(([a-zA-Z])([-\w]+\.)+([^\s\.]+[^\s]*)+[^,.\s])@';
			$content = preg_replace($url, '<a href="http$2://$4" target="_blank" title="$0">$0</a>', $content);	
			//get name from users
			$sqluploader = "SELECT * FROM users WHERE token='$uploader'";
			$resuploader = mysqli_query($db, $sqluploader);
			$rowuploader = $resuploader->fetch_assoc();
			$uploader = $rowuploader['name'];
			$token = $rowuploader['token'];
			$out_uploader = substr($token, 0, -25) . "_" . $uploader;
			$downloads = $row['downloads'];	
			$image = $row['image'];	
			//get file size
			$filesize = filesize($audio); // bytes
...
<label for="plays" onclick='togglePlay()'>
<img class="coffee" height="300px" width="600px" src="<?php echo $image;?>" style="display:none;"></img>
<div class="player" style="display:none" onclick='togglePlay()'>
<img class='image'></img>
 <p class="title"><?php echo $title?></p>
  <div class="overlay"></div>
  <div class="button"><button id="plays" class="play" onclick='togglePlay()'></button><audio oncontextmenu="return false;" style="width:595px;height:40px;margin-top:72px" controlsList="nodownload" id="myAudio" src="<?php echo $audio?>" controls>
...
if(!empty($audio))
{ 
	echo "<script>$('.player').show();</script>";
	
$download = "<form method='post'><button type='submit' name='download' class='btn-sm mb-2 btn-success btn'><i class='fas fa-download'></i> Download <span class='badge badge-light'>$downloads</span></button></form>";
}else{
	$download = false;
	echo "<script>$('.coffee').show();</script>";
	echo "<h5 class='p-1'>$title</h5>";
}
?>
...
.play{
  ...
  cursor: pointer;
  border-style: solid;
  border-width: 37px 1px 40px 60px;
}
...
if(isset($_POST['download']) && $audio == true){
	$download = false;
$visitor_counter = $_SERVER['REMOTE_ADDR']; 
$download_name = $video_name . "_download";	
$sqld = "SELECT * FROM ip_get WHERE get_title='$download_name' ORDER BY id DESC";
$resultd = $connsource->query($sqld);
$rowd = $resultd->fetch_assoc(); 
$ipd = $rowd['ip'];	
echo "<div class='alert alert-success' role='alert'><h4 class='alert-heading'>Your <a href='$audio' name='download' download> Download</a> is ready! <strong>($filesize mb)</strong></h4></div>";
if($ipd != $visitor_counter){
$sqldownload = "INSERT into ip_get (ip, get_title) values ('$visitor_counter', '$download_name')"; 
mysqli_query($db, $sqldownload);	
$sqldownload2 = "UPDATE videos SET downloads = downloads +1 WHERE video_name='$video_name'";
mysqli_query($db, $sqldownload2);
}			
}
//CONTADOR DE DOWNLOADS
...
<div class="p-1">
<form method="post">
<button name="like" type="submit" class="btn-sm btn-info btn mb-2"><i class="fa fa-thumbs-up"> <?php echo $likes?></i></button>
<button name="deslike" type="submit" class="btn-sm btn-info btn mb-2"><i class="fa fa-thumbs-down"> <?php echo $deslikes?></i></button>
<?php echo $download?>

</form>
<?php echo $content; ?><br>
<i class="fa fa-calendar"></i> <?php echo $date ?> 
<i class='text-dark fas fa-user'></i> <a class="text-secondary" href="staff.php?uploader=<?php echo $out_uploader?>"><?php echo $uploader ?></a> 
<i class='fas fa-eye'></i> <?php echo $views_counter ?>
...
while(list(, $val) = each($pieces)){
	...
	<i class="fa fa-hashtag"></i> <a class="text-dark" href="search.php?search=<?php echo $val?>"><?php echo $val?></a>
	<?php
	}
...
if(isset($_POST['comment'])){
		...
		//validation
		$usernames = strip_tags($_POST['namec']);
		$usernames = mysqli_real_escape_string($db, $usernames);
		$usernames = substr($usernames, 0, 20);
		//validation
		$comments = strip_tags($_POST['commentc']);
		$comments = mysqli_real_escape_string($db, $comments);
		$comments = substr($comments, 0, 250);	
		date_default_timezone_set('America/Sao_Paulo');
		$date = date('d M Y\, G:i:s');
		$sqlsource = "INSERT IGNORE into comments (email, name, comment, token, date) VALUES ('$email', '$usernames', '$comments', '$video_name', '$date')";
		
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
		$error = true;
		$error1 = "form-control is-invalid";
		$valid1 = "<div class='invalid-feedback'>Insert a valid email like 'email@gmail.com *</div>";
		}
		
		if(strlen($usernames) < 4 || strlen($usernames) == "" || strlen($usernames) > 20 || !preg_match("/^[a-z0-9._ ]+$/i",$usernames)){
			$error = true;
			$error0 = "form-control is-invalid";
			$valid0 = "<div class='invalid-feedback'>Please insert a valid username and make sure to use only the following characters a-z0-9._</div>";
		}
		
		if(strlen($comments) < 2 || strlen($comments) == "" || strlen($comments) > 250){
		$error = true;
		$error2 = "form-control is-invalid";
		$valid2 = "<div class='invalid-feedback'>The comment section needs to be filed with more than 2 characters</div>";
		}
		
		if($error != true){
			mysqli_query($db, $sqlsource);
			echo "<script>alert('Comment Sended Sucessfully!');</script>";
		}
...
<?php 
		$oldest="?v=$name&oldest";
		$newest="?v=$name";
		$sqlcomment = "SELECT * FROM comments WHERE token='$video_name'";
		$resultcomment = $connsource->query($sqlcomment);
		$comments = $resultcomment->num_rows;
?>
...
<h1 class="lead mt-1 text-white">Leave a Commentary <span class='badge badge-info'><?php echo $comments ?></span></h1>
<form method="post" class="was-validated">
<div class="form-group">
		<input type="text" name="namec" maxlength="20" class="<?php echo $error0?>" id="exampleInputEmail1" aria-describedby="emailHelp" required placeholder="Insert Name *">
		<?php echo $valid0 ?>
		</div>
		<div class="form-group">
        <textarea type="text" name="commentc" class="<?php echo $error2?>" maxlength="250" required placeholder="Comment *"></textarea>
		<?php echo $valid2?>
		</div>
		<div class="form-inline">
		<input type="email" required maxlength="25" name="email" class="<?php echo $error1?> col" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Email *">
		<div class="d-flex justify-content-end">
		<button type="submit" name="comment" class="btn btn-dark ml-2">Submit</button>
		</div>
		<?php echo $valid1?>
		</div>
...
<?php	
		...
		$sqlsource = "SELECT * FROM comments WHERE token='$video_name' ORDER BY id DESC";
		if(isset($_GET['oldest'])){
		$sqlsource = "SELECT * FROM comments WHERE token='$video_name' ORDER BY id ASC";
		}		
...
<div class="dropdown-menu">
    <a class="dropdown-item" href="<?php echo $oldest?>">Oldest</a>
    <a class="dropdown-item" href="<?php echo $newest?>">Newest</a>
  </div>
	
  <button type="button" class="ml-1 btn btn-sm btn-info dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
    Order<span class="sr-only">Toggle Dropdown</span>
  </button>
  </div>
<?php	
		}	
		while($rowsource = $resultsource->fetch_assoc()){
		$commentpost = $rowsource['comment'];
		$datepost = $rowsource['date'];
		$namepost = $rowsource['name'];			
?>

<div class="content2 bg-dark p-1 text-white" style="display:none;">
<?php echo $namepost?> <span class='text-info'>(<?php echo $datepost?>)</span><br /><?php echo $commentpost?>
</div>
<?php
		}
?>
...
//CONTADOR DE VIEWS AND LIKE/DESLIKE
$visitor_counter = $_SERVER['REMOTE_ADDR'];                                                                                                                                                         
$sql = "SELECT * FROM ip_get WHERE get_title='$video_name' ORDER BY id DESC";
$result = $connsource->query($sql);
$row = $result->fetch_assoc(); 
$ip = $row['ip'];
$titleip = $row['get_title'];
if($ip != $visitor_counter){                                                                                  
$sql2 = "INSERT into ip_get (ip, get_title) values ('$visitor_counter', '$video_name')"; 
mysqli_query($db, $sql2);                                                                
$sql3 = "UPDATE videos SET views_counter = views_counter +1 WHERE video_name='$video_name'";  
...
//IF LIKE INSERTED = REMOVE
$out_token2 = substr($video_name,0,6);
$visitor_counter = $_SERVER['REMOTE_ADDR'];                                                                                                                                                         
$sql2 = "SELECT * FROM ip_get WHERE get_title='$out_token2' ORDER BY id DESC";
$result2 = $connsource->query($sql2);
$row2 = $result2->fetch_assoc(); 
$titleip2 = $row2['get_title'];

//LIKE
$out_token = substr($video_name,0,7);
$visitor_counter = $_SERVER['REMOTE_ADDR'];                                                                                                                                                         
$sql = "SELECT * FROM ip_get WHERE get_title='$out_token' ORDER BY id DESC";
$result = $connsource->query($sql);
$row = $result->fetch_assoc(); 
$ip = $row['ip'];
$titleip = $row['get_title'];
if(isset($_POST['like'])){
	if($ip != $visitor_counter){                                                                                  
	$sql2 = "INSERT into ip_get (ip, get_title) values ('$visitor_counter', '$out_token')"; 
	mysqli_query($db, $sql2);                                                                
	$sql3 = "UPDATE videos SET likes = likes +1 WHERE video_name='$video_name'";  
	mysqli_query($db, $sql3);
	
}else{
	$sql4 = "DELETE FROM ip_get WHERE get_title='$out_token'";
	mysqli_query($db, $sql4);
	$sql5 = "UPDATE videos SET likes = likes -1 WHERE video_name='$video_name'";
	mysqli_query($db, $sql5);
}
if($ip != $visitor_counter && $out_token2 == $titleip2 && $deslikes > 0){
	$sql40 = "DELETE FROM ip_get WHERE get_title='$out_token2'";
	mysqli_query($db, $sql40);
	$sql50 = "UPDATE videos SET deslikes = deslikes -1 WHERE video_name='$video_name'";
	mysqli_query($db, $sql50);	
}
//DESLIKE
}
$out_token2 = substr($video_name,0,6);
$visitor_counter = $_SERVER['REMOTE_ADDR'];                                                                                                                                                         
$sql = "SELECT * FROM ip_get WHERE get_title='$out_token2' ORDER BY id DESC";
$result = $connsource->query($sql);
$row = $result->fetch_assoc(); 
$ip = $row['ip'];
$titleip2 = $row['get_title'];
if(isset($_POST['deslike'])){
	if($ip != $visitor_counter){                                                                                  
	$sql2 = "INSERT into ip_get (ip, get_title) values ('$visitor_counter', '$out_token2')"; 
	mysqli_query($db, $sql2);                                                                
	$sql3 = "UPDATE videos SET deslikes = deslikes +1 WHERE video_name='$video_name'";  
	mysqli_query($db, $sql3);	
}else{
	$sql4 = "DELETE FROM ip_get WHERE get_title='$out_token2'";
	mysqli_query($db, $sql4);
	$sql5 = "UPDATE videos SET deslikes = deslikes -1 WHERE video_name='$video_name'";
	mysqli_query($db, $sql5);
}
if($ip != $visitor_counter && $out_token == $titleip && $likes > 0){
	$sql40 = "DELETE FROM ip_get WHERE get_title='$out_token'";
	mysqli_query($db, $sql40);
	$sql50 = "UPDATE videos SET likes = likes -1 WHERE video_name='$video_name'";
	mysqli_query($db, $sql50);	
}
}
//CONTADOR DE VIEWS AND LIKE/DESLIKE

...
if(isset($_POST['subscribe']) || isset($_GET['token'])){	
...
<div align="center" class="lds-dual-ring span12 text-center p-2">
<h1 class="text-danger" style="text-shadow: 1px 1px black;text-align:center;font-size:6.5vw;font-variant: small-caps;">Loading...</h1>
</div>
<div class='alert alert-danger' role='alert' id='al1' style='display:none'>
Your account is alredy subscribed. Please try another one or check your mail box.
</div>
<div class='alert alert-info' id='al2' role='alert' style='display:none'>
Everything setled up! Please check your mailbox to confirm your subscription.
</div>
<div class='alert alert-warning' id='al3' role='alert' style='display:none'>
  You have already activated <strong>This email</strong>!
</div>
<div class='alert alert-success' id='al4' role='alert' style='display:none'>
  Your email is now active!
</div>
<div class='alert alert-danger' id='al5' role='alert' style='display:none'>
 Token not found.
</div>
</div> 
</div>
...
$token = $_GET['token'];
$sqlsources = "SELECT * FROM subscribe WHERE token='$token'";
if($resultsources = $db->query($sqlsources)){
$rowsources = $resultsources->fetch_assoc();
$tokencheck = $rowsources['status'];
$tokenname = $rowsources['token'];
$emailname = $rowsources['email'];
if($tokencheck == "active"){	
?>
<script>
$('#al3').show();
</script>
<?php
}elseif(isset($tokenname)){
$sql = "UPDATE subscribe SET status='active' WHERE token='$tokenname'";
mysqli_query($db, $sql);	
?>
<script>
$('#al4').show();
...
$status = "inactive";
$sql = "INSERT INTO subscribe (email, status, token) VALUES('$email', '$status', '$token')";	
...
$sqlsource = "SELECT * FROM subscribe WHERE email='$email'";
$resultsource = $db->query($sqlsource);
$rowsource = $resultsource->fetch_assoc();
$namecheck = $rowsource['email'];
if($email == $namecheck){
$error = true;	
$form = "form-control is-invalid";
?>	
...
<body>
<h1>Now you're officialy subbed to <span style='color:red'> THE OFFICIAL PODCAST</span>.</h1>
<p>From now on you will be notified when a podcast comes up.</p>
<a href='$link'>Click Here</a> to confirm your subscription.
</body>
</html>
";
